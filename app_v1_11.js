...
(function(){
  ...
  function lsGet(key){ try{ return lsGet(key); }catch(e){ return null; } }
  function lsSet(key,val){ try{ lsSet(key,val); }catch(e){} }
  function lsDel(key){ try{ lsDel(key); }catch(e){} }

  function loadLocal(){
    State.mastery = safeJsonParse(lsGet(LS.mastery), {}) || {};
    State.settings = safeJsonParse(lsGet(LS.settings), State.settings) || State.settings;
    State.leaderboard = safeJsonParse(lsGet(LS.leaderboard), []) || [];
    const initials = lsGet(LS.initials) || '';
    const ii=$('#initialsInput'); if(ii) ii.value = initials;
  }

  function saveLocal(){
    lsSet(LS.mastery, JSON.stringify(State.mastery||{}));
    lsSet(LS.settings, JSON.stringify(State.settings||{}));
    lsSet(LS.leaderboard, JSON.stringify(State.leaderboard||[]));
  }
...
Object.entries(panes).forEach(([k,el])=>{ el.style.display = (k===showKey) ? 'block' : 'none'; });
  ...
  function finishExam(){
    ...
    if(initials) lsSet(LS.initials, initials);
  ...
  async function init(){
    try{
      ...
      showBanner('Init error: ' + String(e.message||e), 'bad');
